...
export default function QRGeneratorModal({ clanItem, onClose }) {
  ...
  // ── Fix: debounce text, KHÔNG gọi setState trực tiếp trong effect body ──
  useEffect(() => {
    clearTimeout(debounceTimer.current);
    debounceTimer.current = setTimeout(() => {
      setDebouncedText(
        `https://giaphaviet.top/pages/detail?id=${clanItem.clanId}`,
      );
    }, 500);
  ...
  const qrUrl = debouncedText
    ? `https://api.qrserver.com/v1/create-qr-code/?size=${finalSize}x${finalSize}&data=${encodeURIComponent(debouncedText)}&ecc=${errorLevel}&margin=1`
    : null;

  // Dùng key để buộc img re-mount khi URL đổi — thay thế cho setState trong effect
  const imgKey = qrUrl ?? "empty";
  ...
  const handleErrorLevelChange = (val) => {
    setErrorLevel(val);
    setQrReady(false);
    setLoading(!!debouncedText);
  };
  ...
  const handlePrint = () => {
    if (!qrReady || !debouncedText) return;
    ...
    // Dùng iframe ẩn thay vì window.open để tránh bị trình duyệt chặn popup
    ...
    const content = `<!DOCTYPE html><html><head><title>In mã QR</title>
      ...
      <body><div class="wrap">
        <img src="${qrUrl}" width="${finalSize}" height="${finalSize}" />
        ${labelHtml}
      </div></body></html>`;

    iframe.srcdoc = content;

    // Đợi iframe load xong rồi mới gọi print
    iframe.onload = () => {
      try {
        iframe.contentWindow.focus();
        iframe.contentWindow.print();
      } catch (err) {
        // Fallback nếu iframe bị chặn: mở tab mới
  ...
  const handleDownload = () => {
    ...
    a.href = qrUrl;
    a.download = `qr-code-${finalSize}px.png`;
    a.target = "_blank";
    a.click();
  };
  ...
  const handleSubmitPrint = (e) => {
    e.preventDefault();
    // const newErrors = {};
    // if (!text.trim()) newErrors.text = "Vui lòng nhập nội dung để tạo mã QR";
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-[100]">
      ...
      {/* Modal — phong cách giấy cổ giống UpdateMemberModal */}
      <motion.div
      ...
      >
        ...
        <form onSubmit={handleSubmitPrint} className="space-y-5 font-serif">
          {/* Nội dung */}
          {/* <div>
            ...
            <textarea
              autoFocus
              rows={3}
              className="w-full px-4 py-2 bg-[#f4ece1] border border-[#8b5a2b]/40 focus:border-[#3d2611] outline-none text-[#3d2611] text-sm transition-all resize-none placeholder:italic placeholder:text-[#8b5a2b]/50"
              placeholder="Nhập URL, địa chỉ ví, văn bản..."
              value={text}
              onChange={(e) => handleTextChange(e.target.value)}
            />
            <div className="flex justify-between items-center mt-0.5">
              <span className="text-[10px] text-[#8b5a2b]/60">
                {text.length} ký tự
              </span>
              {text && (
                <button
                  ...
                  className="text-[10px] text-[#8b5a2b] hover:text-[#3d2611] uppercase font-bold"
                >
                  ✕ Xoá
                </button>
              )}
            ...
            <div className="flex gap-2">
              {ERROR_LEVELS.map(({ value, label }) => (
                <button
                  key={value}
                  type="button"
                  onClick={() => handleErrorLevelChange(value)}
          ...
          {/* Preview */}
          {
            <div className="border border-[#8b5a2b]/30 bg-[#f4ece1] p-4 flex flex-col items-center gap-2">
              ...
              <div className="relative inline-block p-2 bg-white border border-[#8b5a2b]/20 shadow">
                ...
                {/* key buộc img re-mount khi URL đổi — tránh dùng setState trong effect */}
                <img
                  key={imgKey}
                  src={qrUrl}
                  alt="QR preview"
